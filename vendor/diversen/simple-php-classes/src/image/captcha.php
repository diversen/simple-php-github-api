...
// public function MakeCaptcha() {	
        ...
        $bgcolor = imagecolorallocate(
                $image, hexdec(substr($this->_capBgColor, 0, 2)), hexdec(substr($this->_capBgColor, 2, 2)), hexdec(substr($this->_capBgColor, 4, 2))
        );

        $stringcolor = imagecolorallocate(
                $image, hexdec(substr($this->_capTextColor, 0, 2)), hexdec(substr($this->_capTextColor, 2, 2)), hexdec(substr($this->_capTextColor, 4, 2))
        );
        $linecolor = imagecolorallocate($image, 0, 0, 0);
        imagettftext($image, $this->_capCharWidth, 0, 0, 18, $stringcolor, $this->_capFont, $this->_capString);

        switch ($this->_capImageType) {
            case 'jpeg': imagejpeg($image);
                break;
